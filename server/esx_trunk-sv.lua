          ...
          for i = 1, #coffre, 1 do
            if coffre[i].name == item then
              if (coffre[i].count >= count and count > 0) then
                ...
                exports.erylcchi_logger:discord('**'..GetPlayerName(xPlayer.source)..'** took '..count..' '..item..' from vehicle with plate # '..plate, xPlayer.source, 0, '9126339', 'trunklogs')
        ...
        TriggerEvent(
          ...
          function(store)
            ...
            if (getTotalInventoryWeight(plate) + (getItemWeight(item) * count)) > max then
              ...
              exports.erylcchi_logger:discord('**'..GetPlayerName(xPlayer.source)..'** stored '..count..' '..item..' into vehicle with plate # '..plate, xPlayer.source, 0, '5534659', 'trunklogs')
